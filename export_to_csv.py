...
if __name__ == "__main__":
    ...
    csv_file = "/Users/bystanders/Desktop/公共资源交易/招标数据.csv"
    with open(csv_file, 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.writer(f)
        writer.writerow(['公告标题', '项目代码', '发布时间', '投标单位', '报价（元）'])
        ...
        for page in range(1, 3):
            ...
            for item in data["pageData"]:
                try:
                        ...
                        if col.get("viewStyle") == "richText":
                            ...
                            if prices:
                                for price_info in prices:
                                    unit = price_info.get('单位名称', '')
                                    price = price_info.get('报价（元）', price_info.get('报价', ''))
                                    writer.writerow([
                                        item['noticeTitle'],
                                        item['projectCode'],
                                        item['publishDate'],
                                ...
                                writer.writerow([item['noticeTitle'], item['projectCode'], item['publishDate'], '', ''])
                            break
                    else:
                        writer.writerow([item['noticeTitle'], item['projectCode'], item['publishDate'], '', ''])
                    ...
                    print(f"  错误: {item['noticeTitle'][:30]}... - {e}")
